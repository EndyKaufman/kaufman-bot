...
export class BaseBotServer implements IBotServer {
  ...
  public startPlugin(message: string, pluginName: string, locale: string): EventEmitter {
    ...
    for (i = 0; i < len; i++) {
      if ((!founded && pluginName === null && this.plugins[i].check(this.bot, msg)) || (pluginName !== null && this.plugins[i].name === pluginName)) {
        founded = true;
        this.plugins[i]
          .process(this.bot, msg)
  ...
  protected processHook() {
    ...
    this.events.on('message', (msg: IBotMessage, data: any, stop: boolean) => {
      if (data) {
        ...
        const r = /\\u([\d\w]{4})/gi;
        text = text.replace(r, (_match: any, grp: any) => {
