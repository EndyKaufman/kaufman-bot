...
// todo: now disable because not work in github, enable later
describe('GoogleApisBotPlugin', () => {
  describe('without bot', () => {
    let plugin: GoogleApisBotPlugin;
    before(() => {
      config();
      plugin = new GoogleApisBotPlugin('en', ['bot'], process.env.GOOGLE_APIS_API_KEY, process.env.GOOGLE_APIS_HABR_CUSTOM_SEARCH_ENGINE_ID, '', ['habr']);
    });
    it('should response include a message "Angular" or error with text "dailyLimitExceeded"', (done) => {
      ...
      plugin.process(null, msg).on('message', (answer: any) => {
