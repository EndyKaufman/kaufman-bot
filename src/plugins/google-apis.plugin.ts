...
export class GoogleApisBotPlugin extends BaseBotPlugin {
  ...
  public whatCanIdo = {
    en:
      'I know how to look for information on the sites over google custom search api',
    ru:
      'Умею искать информацию на сайтах через поисковое апи google custom search',
  };
      ...
      customsearch.cse.list(options, (error: any, resp: any) => {
        if (error) {
          ...
          if (resp.data.items && resp.data.items.length > 0) {
            const index: number = _.random(
              Math.min(options.num, resp.data.items.length)
            );
