...
export class ScraperBotPlugin extends BaseBotPlugin {
  ...
  public check(bot: IBot, msg: IBotMessage): boolean {
    return (
      msg.text &&
      ((checkWordsInMessage(msg.text, this.wordsForSpy) && msg.chat.type === 'private') ||
        (checkWordsInMessage(msg.text, this.botNameAliases) && checkWordsInMessage(msg.text, this.wordsForSpy) && msg.chat.type !== 'private'))
    );
  }
    ...
    try {
      ...
      request.get(url, options, (error: any, response: any, body: any) => {
        if (error) {
          ...
          let content = this.scraperContentSelector
            .split(',')
          ...
          if (enc !== 'utf8') {
            content = encoding.convert(Buffer.from(content, 'binary'), 'utf8', enc, true).toString('utf8');
          }
  ...
  public process(bot: IBot, msg: IBotMessage): EventEmitter {
    ...
    if (msg.text) {
      ...
      this.scrap(text, msg)
        ...
        .on('customError', (message: string) => event.emit('customError', message));
    }
    return event;
  }
}
